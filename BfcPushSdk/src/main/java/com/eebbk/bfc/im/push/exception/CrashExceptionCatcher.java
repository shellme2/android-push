 ...
 */
public class CrashExceptionCatcher implements UncaughtExceptionHandler {


    // 系统默认的UncaughtException处理类
    private UncaughtExceptionHandler mDefaultHandler;
    // CrashHandler实例
    private static CrashExceptionCatcher INSTANCE;
    // 程序的Context对象
     ...
     */
    public static CrashExceptionCatcher getInstance() {
        if (INSTANCE == null) {
            synchronized (CrashExceptionCatcher.class) {
                if (INSTANCE == null) {
                    INSTANCE = new CrashExceptionCatcher();
                }
            }
        }
        return INSTANCE;
    ...
    private boolean handleException(Throwable ex) {
        ...
        // 使用Toast来显示异常信息
        SyncToastUtil.showToast(mContext, "推送SDK异常崩溃");
        // 收集设备参数信息
    ...
    private String saveCrashInfo2File(Throwable ex) {
        ...
        try {
            ...
            String dirName = temp[0] + "/";// 按日期存放异常日志文件
            String fileName = temp[1] + ".txt";
            if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
                String path = Environment.getExternalStorageDirectory().getPath()
                        + "/xtcdata/logs/sync/crash/" + dirName;
